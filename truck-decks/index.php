<!DOCTYPE html>
<html>
<head>
	<!-- Global Meta -->
	<?php require $_SERVER['DOCUMENT_ROOT'] . '/views/partials/globalMeta.php'; ?>
	<!-- End Global Meta -->
	<title>Decked, LLC provides truck rubber bedliners and storage for pickups.</title>
	<meta name="description" content="Decked, LLC manufactures a storage box that revolutionizes efficiency in your truck bed, boat, and more. Pickup bed liners and storage are our specialty.">
	<meta name="keywords" content="truck bed liners and decks, bed liner, bedliner, truck mats, truck bed liner, spray on bedliner, rubber pickup bed mats, truck boxes, truck storage">
</head>
<body>
	
	<header>
		
		<!-- Main Navigation -->
		<?php require $_SERVER['DOCUMENT_ROOT'] . '/views/partials/mainNavigation.php'; ?>
		<!-- End Main Navigation -->

	</header>

	<main role="main">
		<div class="wrapper mtm">		
			<h1 class="primary-heading underline-heading paragon-text mtn mbxs">Truck Decks</h1>
			<img src="/images/truck-deck.jpg">

			<div class="gd-row gt-row">
				<div class="gd-half gd-columns gt-half gt-columns">
					<div class="content-container mts">
						<img class="gm-full" src="/images/truck-deck-work.jpg">
						<h3 class="primary-heading canon-text mtxs">Work</h3>
						<p>Storing and accessing your stuff needs to be easy. We found a system for our Treasure Valley community which raises the deck, allowing you to store your tools, hardware and other items securely, out of the way, and out of the weather while having full use of the rest of your truck bed. Your time is money and being more efficient and working with less effort allows you more time and energy off the clock to do what you really love.</p>
					</div>
				</div>
				<div class="gd-half gd-columns gt-half gt-columns">
					<div class="content-container mts">						
						<img class="gm-full" src="/images/truck-deck-play.jpg">
						<h3 class="primary-heading canon-text mtxs">Play</h3>
							<p>It can take a hell of a lot of toys to have a lot of fun. With the DECKED system, you can store your rods, guns, skis, boards, sleeping bags, camp gear, tailgate party "items" or whatever else you need. Whether it be at the lake, ocean, mountains or parking lot you can pretty much bring everything – including your girlfriend. Have a cold one on the top, lock your aspirin underneath, park your four-wheeler above and stash your pride below. It’s up to you, fun hog. How are you going to use your DECKED system?</p>
					</div>
				</div>
			</div>
		</div>
		


	</main>

	<footer>

		<!-- Footer -->
		<?php require $_SERVER['DOCUMENT_ROOT'] . '/views/partials/footer.php'; ?>
		<!-- End Footer -->

	</footer>

	<!-- Loading Scripts -->
	<?php require $_SERVER['DOCUMENT_ROOT'] . '/views/partials/scripts.inc.php'; ?>
	<!-- End Loading Scripts -->

</body>
</html>